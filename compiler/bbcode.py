...
def gen_ruby_bbcode(match: Match) -> str:
    """Convert matched ruby tex code into bbcode for bbs usage
    \ruby{A}{B} -> [ruby=B]A[/ruby]

    Also support | split ruby
    \ruby{椎名|真昼}{しいな|まひる} -> [ruby=しいな]椎名[/ruby][ruby=まひる]真昼[/ruby]
    """
    return ''.join('[ruby={}]{}[/ruby]'.format(*pair) for pair in
                   zip(match.group('ruby').split('|'), match.group('word').split('|')))
...
def format_line(source_line: str, fts: FootnoteStorage, ofs: OFootnoteStorage,
                ruby_func: Callable[[Match], str] = gen_ruby_bbcode) -> str:
    ...
    processed_line = re.sub(r'\\gray{(?P<word>.+?)}', '[color=Gray]\\g<word>[/color]', processed_line)
    ...
    processed_line = re.sub(r'\\ruby{(?P<word>.+?)}{(?P<ruby>.+?)}', ruby_func, processed_line)
...
def compile(tex, ruby_available=True, sub_characters=None):
    fts = FootnoteStorage()
    ...
    for line in (s for s in sl[1:] if s):
        if '\\' not in line:
            ...
            if ruby_available:
                ...
                ol.append(format_line(line, fts, ofs, gen_ruby_plain_text))
    if not ol:
        raise EmptyTexError()
    content = '\n'.join(ol)
    if ofs:
